...
/// struct for typed errors of method [`retrieve_account_connection`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RetrieveAccountConnectionError {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom(
                "Received `text/plain` content type response that cannot be converted to `models::AccountConnectionDetailResponse`",
            ))),
            ContentType::Unsupported(unknown_type) => {
                Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::AccountConnectionDetailResponse`"
                ))))
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<RetrieveAccountConnectionError> = serde_json::from_str(&content).ok();
