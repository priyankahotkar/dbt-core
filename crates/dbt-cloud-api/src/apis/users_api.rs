...
pub async fn list_user_credentials(
    ...
    user_id: i32,
) -> Result<models::UserCredentialsListEnveloped, Error<ListUserCredentialsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom(
                "Received `text/plain` content type response that cannot be converted to `models::UserCredentialsListEnveloped`",
            ))),
            ContentType::Unsupported(unknown_type) => {
                Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::UserCredentialsListEnveloped`"
                ))))
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUserCredentialsError> = serde_json::from_str(&content).ok();
