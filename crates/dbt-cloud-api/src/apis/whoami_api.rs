...
) -> Result<models::WhoamiResponse, Error<WhoamiError>> {
    let uri_str = format!("{}/api/v2/whoami", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom(
                "Received `text/plain` content type response that cannot be converted to `models::WhoamiResponse`",
            ))),
            ContentType::Unsupported(unknown_type) => {
                Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::WhoamiResponse`"
                ))))
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<WhoamiError> = serde_json::from_str(&content).ok();
