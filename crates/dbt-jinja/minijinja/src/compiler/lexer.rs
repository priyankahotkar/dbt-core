...
impl<'s> Tokenizer<'s> {
    ...
    // TODO(alex): One day, this will be deprecated
    pub fn force_advance(&mut self, bytes: usize) -> Result<(), Error> {
        if self.rest_bytes().len() < bytes {
        ...
        match marker {
            StartMarker::Comment => {
                if let Some(end) = memstr(&self.rest_bytes()[skip..], self.comment_end().as_bytes())
                {
                    ...
                    Ok(ControlFlow::Break((Token::Comment, self.span(old_loc))))
                } else {
    ...
    fn tokenize_block_or_var(
        ...
        match rest.chars().position(|x| !x.is_whitespace()) {
            Some(0) => {}
            None => {
            ...
            Some(offset) => {
                let skipped = rest.chars().take(offset).map(|c| c.len_utf8()).sum();
                self.advance(skipped);
                return Ok(ControlFlow::Continue(()));
            }
        }
        ...
        match sentinel {
            BlockSentinel::Block => {
                    ...
                    && rest[1..].starts_with(self.block_end())
                {
                    ...
                    let mut span = self.span(old_loc);
                    if was_minus {
                        self.trim_leading_whitespace = true;
                        self.skip_whitespace();
                        span = self.span(old_loc);
                    }
            ...
            BlockSentinel::Variable => {
                    ...
                    && rest[1..].starts_with(self.variable_end())
                {
                    ...
                    if self.brace_balance > 0 {
                        ...
                        self.advance(1);
                        return Ok(ControlFlow::Break((Token::BraceClose, self.span(old_loc))));
                    }
                    return Ok(ControlFlow::Break((Token::VariableEnd, span)));
                }
                if rest.starts_with(self.variable_end()) {
                    // if we are in a dictionary we need to pop the brace first
                    if self.brace_balance > 0 {
                        self.brace_balance -= 1;
                        self.advance(1);
                        return Ok(ControlFlow::Break((Token::BraceClose, self.span(old_loc))));
        ...
        // for brace we need to consider both variable and dictionary syntax
        macro_rules! with_brace_balance {
            ($delta:expr, $tok:expr) => {{
                with_paren_balance!($delta, $tok);
                // we should be within a expression at this point
                self.brace_balance += $delta;
                Some($tok)
            }};
        }
        ...
        let op = match rest.as_bytes().get(0) {
            ...
            Some(b'{') => with_brace_balance!(1, Token::BraceOpen),
            Some(b'}') => with_brace_balance!(-1, Token::BraceClose),
            Some(b'\'') => {
...
#[cfg(test)]
mod tests {
    use crate::compiler::parser::TokenStream;
    ...
    #[test]
    fn test_nonbreak_char() {
        let s = r#"{{config(  materialized='table'  )}}
        ...
        while let Ok(Some((token, _span))) = token_stream.next() {
            tokens.push(token);
        }
        ...
        for (token, expected_token) in tokens.iter().zip(expected_tokens.iter()) {
            match (token, expected_token) {
                (Token::VariableStart, Token::VariableStart) => {}
                (Token::Ident(a), Token::Ident(b)) if a == b => {}
                (Token::ParenOpen, Token::ParenOpen) => {}
                (Token::Assign, Token::Assign) => {}
                (Token::Str(a), Token::Str(b)) if a == b => {}
    ...
    fn test_block_end_span() {
        ...
        while let Ok(Some((token, span))) = token_stream.next() {
