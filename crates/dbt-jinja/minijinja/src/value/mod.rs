...
use self::mutable_map::MutableMap;
...
pub type ValueMap = std::collections::BTreeMap<Value, Value>;
...
#[inline(always)]
pub(crate) fn mutable_map_with_capacity(capacity: usize) -> MutableMap {
    #[cfg(not(feature = "preserve_order"))]
    {
        let _ = capacity;
        MutableMap::new()
    ...
    #[cfg(feature = "preserve_order")]
    {
        MutableMap::with_capacity(crate::utils::untrusted_size_hint(capacity))
    }
}
...
impl Value {
            ...
            + 'static,
    {
                ...
                + 'static,
        {
            ...
            fn enumerate(self: &Arc<Self>) -> Enumerator {
                ...
                let object = DynObject::new(self.clone());
                Enumerator::Iter(Box::new(Iter {
                    iter,
                    _object: object,
                }))
            }
    ...
    /// Creates a callable value from a Rust function that handles parsing and type checking of
    /// arguments dynamically.
    ///
    /// Instead of relying on the runtime-information constrained code generated by
    /// [Value::from_function].
    ///
    /// Use `from_func_func` for faster Rust code compilation, and better error
    /// messages when you parse arguments using [ArgsIter](crate::arg_utils::ArgsIter).
    ///
    /// Additionally, Rust functions can't express all the ways functions can be called
    /// in Python, so this function is more flexible and enables users to write functions
    /// that beahve like Python functions.
    pub fn from_func_func(
        name: &'static str,
        f: impl Fn(&State, &[Value]) -> Result<Value, Error> + Sync + Send + 'static,
    ) -> Value {
        functions::BoxedFunction::from_func_func(
            Arc::new(f),
            #[cfg(feature = "debug")]
            name,
        )
    ...
    /// Returns `true` if this value is internally mutable.
    pub fn is_mutable(&self) -> bool {
        match self.0 {
            ValueRepr::Object(ref obj) => obj.is_mutable(),
            _ => false,
        }
    }
    ...
    ///
    /// ```
    /// # use minijinja::{Environment, value::{Value, Kwargs}, listener::DefaultRenderingEventListener};
    /// # use std::rc::Rc;
    ...
    ///
    /// ```
    /// # use minijinja::{Environment, args, value::{Value, Kwargs}, listener::DefaultRenderingEventListener};
    /// # use std::rc::Rc;
    /// # let mut env = Environment::new();
    /// # env.add_template("foo", "").unwrap();
    /// # let tmpl = env.get_template("foo").unwrap();
    ...
    /// use `Value::from((1,))` instead.
    ///
    /// ```
    /// # use minijinja::Value;
    /// let tuple = Value::from_tuple(vec![Value::from(1), Value::from(2)]);
...
impl Iterator for ValueIter {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match self.imp {
            ...
            ValueIterImpl::Chars(_, len, _) => (len, Some(len)),
