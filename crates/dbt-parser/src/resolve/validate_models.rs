...
/// Validates model properties according to the rules ported from Python dbt.
...
///
/// # Returns
/// * `Ok(vec![])` if the model is valid (empty error list)
/// * `Ok(vec![errors])` if there are validation failures
pub fn validate_model(model_props: &ModelProperties) -> FsResult<Vec<FsError>> {
    let mut errors = Vec::new();
...
mod tests {
    ...
    #[test]
    fn test_validate_model_valid_versions() {
        let mut model_props = create_test_model_properties("test_model");
        ...
        let errors = validate_model(&model_props).unwrap();
        assert!(errors.is_empty());
    }

    #[test]
    fn test_validate_model_invalid_latest_version() {
        let mut model_props = create_test_model_properties("test_model");
    ...
    #[test]
    fn test_validate_model_duplicate_versions() {
        let mut model_props = create_test_model_properties("test_model");
    ...
    #[test]
    fn test_validate_model_multiple_errors() {
        let mut model_props = create_test_model_properties("test_model");
